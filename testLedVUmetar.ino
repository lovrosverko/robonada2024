...
#define SAMPLES         1023          // Must be a power of 2
#define SAMPLING_FREQ   5750         // Hz, must be 40000 or less due to ADC conversion time. Determines maximum frequency that can be analysed by the FFT Fmax=sampleF/2.
...
#define LED_VOLTS       3.3             // Usually 5 or 12
#define NUM_BANDS       8            // To change this, you will need to change the bunch of if statements describing the mapping from bins to bands
#define NOISE           500           // Used as a crude noise filter, values below this are ignored
const uint8_t kMatrixWidth = 6;                          // Matrix width
...
void loop() {
  ...
  for (int i = 2; i < (SAMPLES/2); i++){       // Don't use sample 0 and only first SAMPLES/2 are usable. Each array element represents a frequency bin and its value the amplitude.
    ...
    for (int i = 0; i<=8; i++){
      ...
      Serial.println(bandValues[i]+(int)vReal[i]);
    }
    Serial.println("----------");
    delay(100);
    */
  ...
  for (byte band = 0; band < NUM_BANDS; band++) {
    ...
    Serial.print(band); Serial.print(": "); Serial.println(barHeight);
